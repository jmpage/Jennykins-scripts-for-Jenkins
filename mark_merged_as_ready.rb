...
def get_pull_ids(since)
  print "Retrieving merged pull request ids..."
  ids = []
  git.log.since(since).select { |commit| commit.parents.count > 1 }.each do |commit|
...
def mark_artifacts_as_ready(ids)
  puts "Marking resolved artifacts as Ready..."
  ids.each do |id|
    if (id =~ /^DE/) == 0
      type = "defect"
    elsif (id =~ /^US/) == 0
    ...
    begin
      ...
      if schedule_state == "In-Progress"
        print "Artifact #{id} is In-Progress. Marking as Ready..."
        fields = {}
        fields["Ready"] = true
        artifact.update(fields)
        puts "done"
      else
        puts "Artifact #{id} is not In-Progress (#{schedule_state}). Skipping."
      end
...
pull_ids = get_pull_ids(Time.now - 5*60)
resolved_artifact_ids = get_resolved_artifact_ids(pull_ids)
mark_artifacts_as_ready(resolved_artifact_ids)
